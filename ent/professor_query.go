...
type ProfessorQuery struct {
	...
	withChats            *ChatQuery
	withReceivedMessages *MessageQuery
...
func (pq *ProfessorQuery) QueryChats() *ChatQuery {
...
func (pq *ProfessorQuery) QueryReceivedMessages() *MessageQuery {
...
func (pq *ProfessorQuery) Clone() *ProfessorQuery {
	...
	return &ProfessorQuery{
		...
		withReceivedMessages: pq.withReceivedMessages.Clone(),
...
func (pq *ProfessorQuery) WithChats(opts ...func(*ChatQuery)) *ProfessorQuery {
	query := (&ChatClient{config: pq.config}).Query()
...
func (pq *ProfessorQuery) WithReceivedMessages(opts ...func(*MessageQuery)) *ProfessorQuery {
	query := (&MessageClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withReceivedMessages = query
	return pq
}
...
func (pq *ProfessorQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Professor, error) {
	...
	if query := pq.withChats; query != nil {
		if err := pq.loadChats(ctx, query, nodes, nil,
			func(n *Professor, e *Chat) { n.Edges.Chats = e }); err != nil {
			return nil, err
		}
	}
	if query := pq.withReceivedMessages; query != nil {
		if err := pq.loadReceivedMessages(ctx, query, nodes, nil,
			func(n *Professor, e *Message) { n.Edges.ReceivedMessages = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *ProfessorQuery) loadChats(ctx context.Context, query *ChatQuery, nodes []*Professor, init func(*Professor), assign func(*Professor, *Chat)) error {
	...
	query.Where(predicate.Chat(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(professor.ChatsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.professor_chats
		if fk == nil {
			return fmt.Errorf(`foreign-key "professor_chats" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "professor_chats" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (pq *ProfessorQuery) loadReceivedMessages(ctx context.Context, query *MessageQuery, nodes []*Professor, init func(*Professor), assign func(*Professor, *Message)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Professor)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Message(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(professor.ReceivedMessagesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.professor_received_messages
		if fk == nil {
			return fmt.Errorf(`foreign-key "professor_received_messages" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "professor_received_messages" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
