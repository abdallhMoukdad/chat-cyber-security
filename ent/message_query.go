...
type MessageQuery struct {
	...
	withChat     *ChatQuery
	withSender   *StudentQuery
	withReceiver *ProfessorQuery
	withFKs      bool
...
func (mq *MessageQuery) QuerySender() *StudentQuery {
...
func (mq *MessageQuery) QueryReceiver() *ProfessorQuery {
	query := (&ProfessorClient{config: mq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := mq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (mq *MessageQuery) WithSender(opts ...func(*StudentQuery)) *MessageQuery {
	query := (&StudentClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (mq *MessageQuery) WithReceiver(opts ...func(*ProfessorQuery)) *MessageQuery {
	query := (&ProfessorClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (mq *MessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Message, error) {
	...
	if mq.withChat != nil || mq.withSender != nil || mq.withReceiver != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := mq.withSender; query != nil {
		if err := mq.loadSender(ctx, query, nodes, nil,
			func(n *Message, e *Student) { n.Edges.Sender = e }); err != nil {
			return nil, err
		}
	}
	if query := mq.withReceiver; query != nil {
		if err := mq.loadReceiver(ctx, query, nodes, nil,
			func(n *Message, e *Professor) { n.Edges.Receiver = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MessageQuery) loadSender(ctx context.Context, query *StudentQuery, nodes []*Message, init func(*Message), assign func(*Message, *Student)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Message)
	for i := range nodes {
		if nodes[i].student_sent_messages == nil {
			continue
		}
		fk := *nodes[i].student_sent_messages
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "student_sent_messages" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (mq *MessageQuery) loadReceiver(ctx context.Context, query *ProfessorQuery, nodes []*Message, init func(*Message), assign func(*Message, *Professor)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Message)
	for i := range nodes {
		if nodes[i].professor_received_messages == nil {
			continue
		}
		fk := *nodes[i].professor_received_messages
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "professor_received_messages" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
