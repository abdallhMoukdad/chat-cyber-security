...
func (sq *StudentQuery) QueryChats() *ChatQuery {
	query := (&ChatClient{config: sq.config}).Query()
...
func (sq *StudentQuery) QuerySentMessages() *MessageQuery {
...
func (sq *StudentQuery) Clone() *StudentQuery {
	...
	return &StudentQuery{
		...
		withChats:        sq.withChats.Clone(),
		withSentMessages: sq.withSentMessages.Clone(),
...
func (sq *StudentQuery) WithChats(opts ...func(*ChatQuery)) *StudentQuery {
	query := (&ChatClient{config: sq.config}).Query()
...
func (sq *StudentQuery) WithSentMessages(opts ...func(*MessageQuery)) *StudentQuery {
	query := (&MessageClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withSentMessages = query
	return sq
}
...
func (sq *StudentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Student, error) {
	...
	if query := sq.withChats; query != nil {
		if err := sq.loadChats(ctx, query, nodes, nil,
			func(n *Student, e *Chat) { n.Edges.Chats = e }); err != nil {
			return nil, err
		}
	}
	if query := sq.withSentMessages; query != nil {
		if err := sq.loadSentMessages(ctx, query, nodes, nil,
			func(n *Student, e *Message) { n.Edges.SentMessages = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *StudentQuery) loadChats(ctx context.Context, query *ChatQuery, nodes []*Student, init func(*Student), assign func(*Student, *Chat)) error {
	...
	query.Where(predicate.Chat(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(student.ChatsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.student_chats
		if fk == nil {
			return fmt.Errorf(`foreign-key "student_chats" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "student_chats" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (sq *StudentQuery) loadSentMessages(ctx context.Context, query *MessageQuery, nodes []*Student, init func(*Student), assign func(*Student, *Message)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Student)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Message(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(student.SentMessagesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.student_sent_messages
		if fk == nil {
			return fmt.Errorf(`foreign-key "student_sent_messages" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "student_sent_messages" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
