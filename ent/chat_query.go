...
func (cq *ChatQuery) QueryStudent() *StudentQuery {
...
func (cq *ChatQuery) QueryProfessor() *ProfessorQuery {
...
func (cq *ChatQuery) WithStudent(opts ...func(*StudentQuery)) *ChatQuery {
	query := (&StudentClient{config: cq.config}).Query()
...
func (cq *ChatQuery) WithProfessor(opts ...func(*ProfessorQuery)) *ChatQuery {
	query := (&ProfessorClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *ChatQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Chat, error) {
	...
	if cq.withStudent != nil || cq.withProfessor != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := cq.withStudent; query != nil {
		if err := cq.loadStudent(ctx, query, nodes, nil,
			func(n *Chat, e *Student) { n.Edges.Student = e }); err != nil {
			return nil, err
		}
	}
	if query := cq.withProfessor; query != nil {
		if err := cq.loadProfessor(ctx, query, nodes, nil,
			func(n *Chat, e *Professor) { n.Edges.Professor = e }); err != nil {
			return nil, err
		}
	}
	if query := cq.withMessages; query != nil {
		if err := cq.loadMessages(ctx, query, nodes, nil,
			func(n *Chat, e *Message) { n.Edges.Messages = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *ChatQuery) loadStudent(ctx context.Context, query *StudentQuery, nodes []*Chat, init func(*Chat), assign func(*Chat, *Student)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Chat)
	for i := range nodes {
		if nodes[i].student_chats == nil {
			continue
		}
		fk := *nodes[i].student_chats
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "student_chats" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (cq *ChatQuery) loadProfessor(ctx context.Context, query *ProfessorQuery, nodes []*Chat, init func(*Chat), assign func(*Chat, *Professor)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Chat)
	for i := range nodes {
		if nodes[i].professor_chats == nil {
			continue
		}
		fk := *nodes[i].professor_chats
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "professor_chats" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
