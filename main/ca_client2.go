...
func main() {
	...
	// Save CA certificate to file (optional)
	caCertFile, err := os.Create("ca_certificate.pem")
	if err != nil {
		fmt.Println("Error creating CA certificate file:", err)
		return
	}
	pem.Encode(caCertFile, &pem.Block{Type: "CERTIFICATE", Bytes: caCertificate})
	caCertFile.Close()
...
func sendCertificateRequest(conn net.Conn, csr []byte) error {
	err := sendLengthAndData(conn, csr)
	if err != nil {
		return err
	}

	// Wait for a response from the server (CA)
